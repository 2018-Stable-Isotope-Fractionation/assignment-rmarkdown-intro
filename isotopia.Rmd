...
After *isotopia* is installed, it can be loaded at any time like any other R module using ```library(isotopia)```. The basic data types are initialized simply by calling the respective ```ratio```, ```abundance``` and ```delta``` functions with single or multiple values.

```{r data-types}
library(isotopia)
ratio(0.1) %>% print()
abundance(c(0.1, 0.2), notation="raw") %>% print()
delta(100, notation = "permil") %>% print()
```
...
```{r}
ratio(`13C` = 0.011, major = "12C", compound = "CO2") %>% print()
```
...
seq(from = 0.005, to = 0.02, by = 0.001) %>% print()

# use the seq, the pipe operator (%>%), the ratio function and the set_attrib() function to make a ratio for minor isotope 13C and major 12C. At first just print it, then store it in the variable r instead of printing it.
...
d %>% print()
d %>% switch_notation("raw") %>% print()
d %>% switch_notation("ppm") %>% print()
```
